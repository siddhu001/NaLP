import pickle
relation_name_dict=pickle.load(open("relation_name_dict.pkl","rb"))
entity_name_dict=pickle.load(open("entity_name_dict.pkl","rb"))
relation_doub_dict={}
entity_doub_dict={}
for line in open("data/JF17K_version1/test.txt"):
...
pickle.dump(relation_name_dict,open("relation_name_dict_new.pkl","wb"))
pickle.dump(entity_name_dict,open("entity_name_dict_new.pkl","wb"))
file_write=open("p_a_list_test.txt","w")
for k in relation_dict:
	...
	for j in range(len(relation_dict[k])):
		if (j==(len(relation_dict[k])-1)):
			...
			file_write.write(str(relation_dict[k][j])+",")
	file_write.write("\n")
file_write=open("a_p_list_test.txt","w")
